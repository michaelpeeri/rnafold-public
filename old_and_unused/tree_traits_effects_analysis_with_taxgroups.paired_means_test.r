...
cairo_pdf(sprintf("tree_phenotypes_regression.out.%s.by.taxgroups.%%d.pairedMean.pdf", profileMode))
...
performGLSregression_profileRangeMean <- function( traits, tree, Xtrait, profileRange )
{
    ...
    ##if( any(class(traits[,Xtrait])==c("factor")) )
    ##{
    dx <- as.matrix( traits[variables] )
    #print(length(colMeans(dx))) # --> 11
    #print(length(rowMeans(dx))) # --> 169
    positionMeans <- colMeans(dx)
    stopifnot( length(positionMeans) == profileRange[2]-profileRange[1]+1 )
    
    traits$PairedMean <- colMeans(t(dx) - positionMeans)  # use mean differences within range
    #print(rbind( rowMeans(dx), traits$PairedMean) )
    stopifnot( abs( mean(traits$PairedMean) ) < 1e-8 )   # The paired means should now be centered around 0
    #print( cor( rowMeans(dx), traits$PairedMean, method="pearson" ) )
    stopifnot( cor( rowMeans(dx), traits$PairedMean, method="pearson" ) > 0.99 )  # The paired values should be highly correlated with the XXXXXXXXXXXXXXXXXXXXXXXXXX
    print(dx)
    ##}
    ##else
    ##{
    ...
    return( performGLSregression( traits, tree, Xtrait, "PairedMean", plotRegression=FALSE ) )
}
