...
import gff
from word_stats import  OverlappingWordStats
from interval_utils import mergedIntervals
...
# Feature-types of all features that are transcribed (i.e., contain 'mRNA' or 'noncoding_exon')
transcribedParentFeature = frozenset(('gene', 'transposable_element_gene', 'LTR_retrotransposon', 'tRNA_gene', 'ncRNA_gene', 'snoRNA_gene', 'rRNA_gene', 'snRNA_gene', 'telomerase_RNA_gene'))
...
for area in yeastChromosomes:
    ...
    assert(chromosomeBoundaries[0] < chromosomeBoundaries[1] - MinSaneChromosomeLength)
    ...
    for cds in db.features_of_type('CDS', limit=selectedChromosome, completely_within=False):
        ...
        for feat in db.parents(cds.id):
            if feat.featuretype == 'mRNA':
                ...
                print("Found CDS contained in unexpected feature %s of type %s" % (feat.id, feat.featuretype))
                assert(False)
    ...
    for mRNA in db.features_of_type(('mRNA', 'noncoding_exon'), limit=selectedChromosome, completely_within=False):
        ...
        for gene in db.parents(mRNA.id):
            if gene.featuretype in transcribedParentFeature:
                geneId = gene.id
            else:
...
print("..........")
mergedIntervals( codingRegions )
mergedIntervals( nonCodingRegions )
mergedIntervals( transcribedRegions )
mergedIntervals( nonTranscribedRegions )
