...
from data_helpers import decompressNucleicSequence, checkSpeciesExist, getSpeciesFileName
...
for taxid in args.taxid:
    ...
    #with pd.io.pytables.HDFStore("gcdata_v2_taxid_%d_profile_2000_10_begin.h5" % taxid) as store:
    with pd.io.pytables.HDFStore("gcdata_v2_taxid_%d_profile_%d_%d_%s_%d.h5" % (taxid, args.profile[0], args.profile[1], args.profile[2], args.profile[3])) as store:
