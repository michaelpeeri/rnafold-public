...
pdf("profile_multiple_regression_random_forest.2.out.pdf")
...
testRandomTreeParams <- function( formula, data, ... )
{
    ...
    for( mtry in seq(6, 9, 1) )
    {
        #for( nodesize in seq(0.5, 21.5, 3) )
        for( nodesize in seq(3, 12, 1.5) )
        {
...
#block <- testRandomTreeParams( modelFormula, data=traits[ ( !is.na(traits$Profile.26) & !is.na(traits$GenomicGC)  & !is.na(traits$Habitat) & !is.na(traits$OptimumTemp) & !is.na(traits$OxygenReq) & !is.na(traits$ProteinCount) & !is.na(traits$GenomeSizeMb) & !is.na(traits$GenomeDensity) ), ], N=N, M=M, ntree=500 )
