...
pdf("profile_multiple_regression_random_forest.out.3.pdf")
...
{
    out <- data.frame( ntree=integer(), nodesize=double(), pearson=double() )
    ...
    for( ntree in seq(500,2500,500) )
    {
        ...
        for( nodesize in seq(5, 29, 3) )
        {
            print(sprintf("ntree=%d, nodesize=%f", ntree, nodesize))
            stats <- performRegressionWithRepeatedCrossVal( formula, data, mtry=mtry, nodesize=nodesize, ntree=ntree, ... )
            ...
            #out <- rbind( out, data.frame( ntree=c(ntree), nodesize=c(nodesize), pearson=c(ntree*nodesize) ) )  # dummy values
            out <- rbind( out, data.frame( ntree=c(ntree), nodesize=c(nodesize), pearson=c(meanPearson) ) )
        }
    }
    
    p <- ggplot( data=out, aes( x=ntree, y=nodesize, fill=pearson ) ) +
        geom_raster() +
    ...
    optNtree <- out[ which.max( out$pearson ), "ntree" ]

    outOptNtree <- out[ out$ntree==optNtree, ]
    p <- ggplot( data=outOptNtree, aes( x=nodesize, y=pearson ) ) +
        geom_step()
    print(p)
...
visualizeRegressor <- function( regressor )
{
    ...
    p <- ggplot() +
        geom_line( data=out[out$GenomicGC==20, ], color="#000000" ) + 
        geom_line( data=out[out$GenomicGC==30, ], color="#000000" ) + 
        geom_line( data=out[out$GenomicGC==40, ], color="#000000" ) + 
        geom_line( data=out[out$GenomicGC==50, ], color="#000000" ) + 
        geom_line( data=out[out$GenomicGC==60, ], color="#000000" ) + 
        geom_line( data=out[out$GenomicGC==70, ], color="#000000" ) + 
        geom_line( data=out[out$GenomicGC==80, ], color="#000000" ) + 
    print(p)
...
#block <- testRandomTreeParams( modelFormula, data=traits[ ( !is.na(traits$Profile.26) & !is.na(traits$GenomicGC)  & !is.na(traits$Habitat) & !is.na(traits$OptimumTemp) & !is.na(traits$OxygenReq) & !is.na(traits$ProteinCount) & !is.na(traits$GenomeSizeMb) & !is.na(traits$GenomeDensity) ), ], N=N, M=M, ntree=1000 )
block <- testRandomTreeParams( modelFormula, data=traits[ ( !is.na(traits$Profile.15) & !is.na(traits$GenomicGC) & !is.na(traits$OptimumTemp) ), ], N=N, M=M )

dev.off()
quit()


#stats <- performRegressionWithRepeatedCrossVal( modelFormula, data=traits[ ( !is.na(traits$Profile.26) & !is.na(traits$GenomicGC)  & !is.na(traits$Habitat) & !is.na(traits$OptimumTemp) & !is.na(traits$OxygenReq) & !is.na(traits$ProteinCount) & !is.na(traits$GenomeSizeMb) & !is.na(traits$GenomeDensity) ), ], N=N, M=M, nodesize=10.75, mtry=9, ntree=2500 )
#stats <- performRegressionWithRepeatedCrossVal( modelFormula, data=traits[ ( !is.na(traits$Profile.26) & !is.na(traits$GenomicGC) & !is.na(traits$OptimumTemp) ), ], N=N, M=M, nodesize=10.75, mtry=9, ntree=5000 )
...
rf <- getRandomForestRegression( modelFormula, data=traits, nodesize=10.75, mtry=9, ntree=5000, na.action="na.omit" )
varImpPlot( rf )
print( importance( rf ) )
